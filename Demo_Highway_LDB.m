...
disp('----------------------------------------------------------------------------');
disp('Computing database LDB descriptors.')
...
for i=1:DBNumberOfFrames
    ...
    LDBFrontMat(i,:)=LDB (ReducedFrontFrame,RegionsMat,ComparisonVector);
    ...
    LDBRearMat(i,:)=LDB (ReducedRearFrame,RegionsMat,ComparisonVector);
    ...
    LDBSide1Mat(i,:)=LDB (ReducedSide1Frame,RegionsMat,ComparisonVector);
    ...
    LDBSide2Mat(i,:)=LDB (ReducedSide2Frame,RegionsMat,ComparisonVector);
end
ProcessingTime=toc;
disp(strcat('Number of database frames =',{' '},num2str(DBNumberOfFrames),' Panoramic frames'));
disp(strcat('Database LDB discriptors computed in',{' '},num2str(ProcessingTime), ' seconds')); 
...
tic
disp('Computing Quary frames LDB descriptors.')
for i=1:RTNumberOfFrames
    ...
    ReducedFrontFrame=imresize(GrayFrontFrame,[ImageSize ,ImageSize]);
    ReducedFrontFrame=LocalNormalize(ReducedFrontFrame,8);  %patch illumination normalization
    RtLDBFrontMat(i,:)=LDB (ReducedFrontFrame,RegionsMat,ComparisonVector);
    ReducedRearFrame=imresize(GrayRearFrame,[ImageSize ,ImageSize]);
    ReducedRearFrame=LocalNormalize(ReducedRearFrame,8);    %patch illumination normalization
    RtLDBRearMat(i,:)=LDB (ReducedRearFrame,RegionsMat,ComparisonVector);
    ReducedSide1Frame=imresize(GraySide1Frame,[ImageSize ,ImageSize]);
    ReducedSide1Frame=LocalNormalize(ReducedSide1Frame,8);  %patch illumination normalization
    RtLDBSide1Mat(i,:)=LDB (ReducedSide1Frame,RegionsMat,ComparisonVector);
    ReducedSide2Frame=imresize(GraySide2Frame,[ImageSize ,ImageSize]);
    ReducedSide2Frame=LocalNormalize(ReducedSide2Frame,8);  %patch illumination normalization
    RtLDBSide2Mat(i,:)=LDB (ReducedSide2Frame,RegionsMat,ComparisonVector);
end
ProcessingTime=toc;
disp(strcat('Number of quary frames =',{' '},num2str(RTNumberOfFrames),' Panoramic frames'));
disp(strcat('Quary frames LDB discriptors computed in',{' '},num2str(ProcessingTime), ' seconds')); 
